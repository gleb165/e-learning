...
class SubjectSerializer(serializers.ModelSerializer):
    ...
    def get_popular_courses(self, obj):
        courses = obj.courses.annotate(total_students=
                                       Count('student')).order_by('total_students')[:3]
        return [
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Course
        fields=['id', 'subject', 'title', 'slug', 'overview', 'created','modules']
...
class ModuleWithContentsSerializer(serializers.ModelSerializer):
    contents = ContendSerializer(many=True)
    class Meta:
        model = Module
